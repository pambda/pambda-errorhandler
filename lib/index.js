  ...
  return next => {
    ...
    return (event, context, callback) => {
      next(event, context, (originalErr, result) => {
        ...
        const TopicArn = errorNotificationArn || (errorNotificationTopicName &&
          `arn:aws:sns:${process.env.AWS_REGION}:${getAccountId(context)}:${errorNotificationTopicName}`
        );
